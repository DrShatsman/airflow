...
dag = DAG('load_currates_daily', default_args=default_args, schedule= '0 7 * * *', catchup=True,
...
clean_temp_table = BashOperator(
    ...
    bash_command='python3 /airflow/scripts/load_currates_daily/clean_temp_table.py',
    dag=dag)

extract_data_and_load_into_temp_table = BashOperator(
    task_id='extract_data_and_load_into_temp_table',
    bash_command='python3 /airflow/scripts/load_currates_daily/extract_data_and_load_into_temp_table.py',
    dag=dag)

check_data_in_the_temp_table = BashOperator(
    task_id='check_data_in_the_temp_table',
    bash_command='python3 /airflow/scripts/load_currates_daily/check_data_in_the_temp_table.py',
    dag=dag)

load_data_into_table = BashOperator(
    task_id='load_data_into_table',
    bash_command='python3 /airflow/scripts/load_currates_daily/load_data_into_table.py',
    dag=dag)

again_clean_temp_table = BashOperator(
    task_id='again_clean_temp_table',
    bash_command='python3 /airflow/scripts/load_currates_daily/again_clean_temp_table.py',
    dag=dag)
